...
# les Fonction pour télécharger les données brutes
def DownloadRawData():
    # URL du fichier ZIP
...
def ReadRawData():
    ...
    data = pd.read_excel(
        data_file, header=1
    )  # header=1 pour ignorer la première ligne d'en-tête
...
def FormattageRawData():
    ...
    rename_dict = {
        x: y
        for x, y in zip(raw_data.loc[:, "pay_0":"pay_amt6"].columns, new_column_names)
    }
...
def plot_distributions(df):
    ...
    plt.figure(
        figsize=(15, 20)
    )  # Adapter la taille de la figure en fonction du nombre de colonnes
...
def plot_discretize_distributions(df, cat_var):
    ...
    for column in columns_to_analyze:
        ...
        plt.xticks(
            rotation=45
        )  # Faire pivoter les étiquettes de l'axe des x pour plus de lisibilité
...
def pct_default_by_category(df, cat_var):
    ...
    ax = (
        df.groupby(cat_var)["default_payment_next_month"]
    ...
    if show_plot:
        ...
        if show_pr_curve:
            metrics.PrecisionRecallDisplay.from_estimator(
                model, X_test, y_test, ax=ax[2], name=""
            )
